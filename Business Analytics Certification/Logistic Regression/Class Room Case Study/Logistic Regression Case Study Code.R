...
#All the values are either continuous or binary. 
...
#Based on the boxplot outliers; capping below 4% & above 96%
...
#Bi-variate analysis just to analyze the data
library(car)
...
#Since all the variable are below the threshold of 5; can't proceed with the model
mod1 <- glm(as.factor(survived) ~ ., family="binomial", data=t_train)
summary(mod1)

#instead of removing all these variables one by one, using the step function, which automatically calculated the best equation
...
#To analyse the confusion matrix and model accuracy
library(lattice)
library(ggplot2)
library(caret)
library(e1071)
